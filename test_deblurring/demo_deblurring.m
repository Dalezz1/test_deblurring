...
%%
filename = 'E:\oof\a01.jpg'; opts.kernel_size =50;  saturation = 0;
lambda_dark = 4e-3; lambda_grad = 4e-3;opts.gamma_correct = 1.0;
lambda_tv = 0.001; lambda_l0 = 5e-4; weight_ring = 1;
%%
% filename = 'pic\real\taotao15fenbianlv.jpg'; opts.kernel_size =30;  saturation = 0;
% lambda_dark = 4e-3; lambda_grad = 4e-3;opts.gamma_correct = 1.0;
% lambda_tv = 0.001; lambda_l0 = 5e-4; weight_ring = 1;
%%
% filename = 'pic\real\img_06.png'; opts.kernel_size = 55;  saturation = 0;
% lambda_dark = 4e-3; lambda_grad = 4e-3;opts.gamma_correct = 1.0;
% lambda_tv = 0.001; lambda_l0 = 5e-4; weight_ring = 1;
%%
% filename = 'pic\real\R9BiCup.jpg'; opts.kernel_size =30;  saturation = 0;
% lambda_dark = 4e-3; lambda_grad = 4e-3;opts.gamma_correct = 1.0;
% lambda_tv = 0.001; lambda_l0 = 5e-4; weight_ring = 1;
%%
...
if size(y,3)==3
    yg = im2double(rgb2gray(y));%%彩色图像转为灰度图像，再转为double类型
else
    yg = im2double(y);%%灰度图像转为double类型
end
tic;
...
k = kernel - min(kernel(:));
k = k./max(k(:));
imwrite(k,['result\' filename(7:end-4) '_kernel.png']);
imwrite(Latent,['result\' filename(7:end-4) '_result.png']);
imwrite(interim_latent,['result\' filename(7:end-4) '_interim_result.png']);
%%
